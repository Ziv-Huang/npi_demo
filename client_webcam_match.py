...
class Client():
    ...
    def ws_connection(self):
        ...
        self.ws = create_connection("ws://10.36.172.193:3001/websocket")
        
        # self.ws = create_connection("ws://10.42.0.249:3001/websocket")
        ...
        while True:
            ...
            if self.ret:
                ...
                for n in pasteNameList:
                    ...
                    draw = ImageDraw.Draw(img_PIL)
                    # npi=0
                    contextPositionOffsetPIL = int((contextScalePIL*(faceW+150)))
                    contextPositionGap = int((contextScalePIL*(faceW+100)))
                    if npi<=2:
                        # draw.text((left,top-int((contextScalePIL*(faceW+50)))), face['Emotion']+' '+symbol.OneLevel, fill = symbol.color[0], font=font)
                        draw.text((left,top-contextPositionOffsetPIL-contextPositionGap), '情緒(emotion) : '+face['Emotion'], fill = symbol.emotionColor, font=font)
